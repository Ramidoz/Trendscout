...
function parseAnswer(raw: string): ParsedSummary | null {
  ...
  const sectionHeaders = /^(key insight|why it matters|suggested direction|top opportunities|action plan|summary|overview|conclusion)[:\s]*$/i;
  const whyPatterns = /\b(because|since|due to|matters|important|significant|key reason|growing|trend)\b/i;
  const directionPatterns = /\b(should|recommend|suggest|consider|try|focus|start|create|post|next step|action|strategy)\b/i;
  ...
  for (const line of lines) {
    if (!line || sectionHeaders.test(line)) continue;

    if (!insight) {
      insight = line;
    } else if (!why && whyPatterns.test(line) && !directionPatterns.test(line)) {
      why = line;
    } else if (!direction && directionPatterns.test(line)) {
      direction = line;
    }
  }
  ...
  const remaining = lines.filter(
    (l) => l && !sectionHeaders.test(l) && l !== insight && l !== why && l !== direction
  );
...
const InteractionPanel = forwardRef<HTMLTextAreaElement, InteractionPanelProps>(
          ...
          {!selectedTopic && opportunities.length > 0 && (
            <>
              <select
                ...
                className="w-full mb-2 px-3 py-2 rounded-lg bg-zinc-800 border border-zinc-700 text-zinc-300 text-sm focus:outline-none focus:border-purple-500/50"
              >
              ...
              {!followUpTopic && (
                <p className="text-xs text-zinc-500 mb-3">
                  Select a topic above to ask a follow-up
                </p>
              )}
            </>
          )}
